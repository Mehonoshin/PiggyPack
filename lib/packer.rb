class Packer
  ...
  def self.pack(filename, hidden_filename, output_filename)
    `convert #{hidden_filename} -compress none tmp.ppm`
    out = File.open("tmp.pgm", "w")
    handle = File.open(filename, "r")
    overlay_handle = File.open("tmp.ppm", "r")
